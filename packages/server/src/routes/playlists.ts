...
router.post('/', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO playlists (user_id, name, description, is_public, cover_url) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [req.user!.id, name, description, isPublic || false, coverUrl],
    );
    res.status(201).json({ playlist: result.rows[0] });
  } catch (error) { console.error('Create playlist error:', error); res.status(500).json({ error: 'Internal server error' }); }
});
  ...
  try {
    let result = await pool.query(
      `SELECT p.id, p.name, p.description, p.cover_url, p.created_at,
              u.username as creator, u.avatar_url as creator_avatar, COUNT(ps.song_id) as song_count
       FROM playlists p JOIN users u ON p.user_id = u.id
       LEFT JOIN playlist_songs ps ON p.id = ps.playlist_id
       WHERE p.is_public = true GROUP BY p.id, u.username, u.avatar_url
       HAVING COUNT(ps.song_id) > 0 ORDER BY COUNT(ps.song_id) DESC LIMIT 20`,
    );
    if (result.rows.length === 0) {
      result = await pool.query(
        `SELECT p.id, p.name, p.description, p.cover_url, p.created_at,
                u.username as creator, u.avatar_url as creator_avatar, 0 as song_count
         FROM playlists p JOIN users u ON p.user_id = u.id
         WHERE p.is_public = true ORDER BY p.created_at DESC LIMIT 20`,
      );
    }
    res.json({ playlists: result.rows });
  } catch (error) { res.status(500).json({ error: 'Internal server error' }); }
...
router.get('/:id', optionalAuthMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (playlistResult.rows.length === 0) { res.status(404).json({ error: 'Playlist not found' }); return; }
    const playlist = playlistResult.rows[0];
    if (!playlist.is_public && (!req.user || req.user.id !== playlist.user_id)) { res.status(403).json({ error: 'Access denied' }); return; }
    const songsResult = await pool.query(
      `SELECT s.id, s.title, s.lyrics, s.style, s.cover_url, s.audio_url, s.duration,
       ...
       FROM playlist_songs ps JOIN songs s ON ps.song_id = s.id JOIN users u ON s.user_id = u.id
       WHERE ps.playlist_id = $1 ORDER BY ps.position ASC`,
      [req.params.id],
    );
    const songs = await Promise.all(songsResult.rows.map(async (row) => ({
...
router.post('/:id/songs', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const check = await pool.query('SELECT user_id FROM playlists WHERE id = $1', [req.params.id]);
    if (check.rows.length === 0) { res.status(404).json({ error: 'Playlist not found' }); return; }
    if (check.rows[0].user_id !== req.user!.id) { res.status(403).json({ error: 'Access denied' }); return; }
    const posResult = await pool.query('SELECT MAX(position) as max_pos FROM playlist_songs WHERE playlist_id = $1', [req.params.id]);
...
router.patch('/:id', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (updates.length > 0) {
      ...
      await pool.query(`UPDATE playlists SET ${updates.join(', ')} WHERE id = $${paramCount}`, values);
    }
